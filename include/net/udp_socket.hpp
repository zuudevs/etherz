...
// ═══════════════════════════════════════════════
//  UdpSocket<Ip<6>> — UDP IPv6
// ═══════════════════════════════════════════════

template <>
class UdpSocket<Ip<6>> {
public:
	...
	core::Error bind(const address_type& addr) noexcept {
		...
		auto& bytes = addr.address().bytes();
		std::copy(bytes.begin(), bytes.end(),
			reinterpret_cast<uint8_t*>(&sa.sin6_addr));
		if (::bind(fd_, reinterpret_cast<struct sockaddr*>(&sa), sizeof(sa)) == impl::socket_error)
			return core::last_platform_error();
		return core::Error::None;
	}
	...
	RecvResult recv_from(std::span<uint8_t> buffer) noexcept {
		...
		std::array<uint16_t, 8> addr_parts{};
		auto* raw = reinterpret_cast<const uint8_t*>(&sender_addr.sin6_addr);
		for (int i = 0; i < 8; ++i) {
			addr_parts[i] = static_cast<uint16_t>(
				(static_cast<uint16_t>(raw[i * 2]) << 8) | raw[i * 2 + 1]);
		}
		result.sender = address_type(
			protocol_type(addr_parts[0], addr_parts[1], addr_parts[2], addr_parts[3],
				addr_parts[4], addr_parts[5], addr_parts[6], addr_parts[7]),
